...
function fillCards() {
  fetch(apiUrl)
    ...
    .then((data) => {
      ...
      for (let k = 0; k < gameTitles.length; k++) {
        // const used to make a unique url for cheapshark to run an api
        const encodedGameTitle = encodeURIComponent(gameTitles[k]);
        ...
        fetch(apiCheapSharkUrl)
          ...
          .then((data) => {
            ...
            for (let j = 0; j < data.length; ++j) {
              ...
              let button_title = "";
              if (k == 0) {
                button_title = "modalbutton";
              } else {
                button_title = `button_${k}`;
              }
              const modalButton = document.getElementById(button_title);
              ...
              modalButton.addEventListener("click", () => {
                ...
                const targetModalId =
                  modalButton.getAttribute("data-mdb-target");
                ...
                fetch(apiCheapSharkDealLookUp)
                  ...
                  .then((data) => {
                    ...
                    const dealPriceElement = document.querySelector(
                      `${targetModalId} #dealPrice`
                    );
                    dealPriceElement.textContent = data.deals[0].price;

                    const retailPriceElement = document.querySelector(
                      `${targetModalId} #retailPrice`
                    );
                    retailPriceElement.textContent = data.deals[0].retailPrice;

                    const dealLinkElement = document.querySelector(
                      `${targetModalId} #dealLink`
                    );
